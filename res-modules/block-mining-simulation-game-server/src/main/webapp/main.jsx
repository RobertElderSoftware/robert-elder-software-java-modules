...
function block_coord_to_cuboid_address(c){
	...
	return new CuboidAddress(
		new Coordinate(
			[
				big_int_divide_and_floor(c.getX(), CHUNK_SIZE) * CHUNK_SIZE,
				big_int_divide_and_floor(c.getY(), CHUNK_SIZE) * CHUNK_SIZE,
				big_int_divide_and_floor(c.getZ(), CHUNK_SIZE) * CHUNK_SIZE,
				BigInt(0)
			]
		),
		new Coordinate(
			[
				(big_int_divide_and_floor(c.getX(), CHUNK_SIZE) * CHUNK_SIZE) + CHUNK_SIZE,
				(big_int_divide_and_floor(c.getY(), CHUNK_SIZE) * CHUNK_SIZE) + CHUNK_SIZE,
				(big_int_divide_and_floor(c.getZ(), CHUNK_SIZE) * CHUNK_SIZE) + CHUNK_SIZE,
				BigInt(0) + BigInt(1)
			]
		)
	);
...
class ChunkModel{
	constructor(){
		this.available_chunks = {},
		this.pending_requested_chunks = {}
	}
...
class Cuboid{
	...
	get_data_for_one_block(x,y,z){
		...
		if(size_of_block > 0){
			...
			return {"size":size_of_block, "data": aggregate_block_data};
		}else{
			return {"size":size_of_block, "data": new ArrayBuffer(0)};
		}
	}
}
...
document.addEventListener("DOMContentLoaded", function(e) {
	...
	class IndividualBlock extends React.Component {
		...
		render(){
			...
			return (
				<td class={classname}>{this.renderBlockValue(this.getBlockValue(new Coordinate([BigInt(this.props.block_x), BigInt(this.props.block_y), BigInt(this.props.block_z), BigInt(0)])))}</td>
			);
		}
	}
	...
	class BlockGrid extends React.Component {
		...
		render() {
			...
			return (
				<table cellspacing={"0"} class={"block-grid-table"}>
					<tbody>{rows}</tbody>
				</table>
			);
		}
	...
	class HomePageUI extends React.Component {
		...
		onSingleBlockWrite(){
			...
			let lower_coordinate = new Coordinate([
				this.state.x - BigInt(1),
				this.state.y,
				this.state.z - BigInt(1),
				BigInt(0)
			]);
			let upper_coordinate = new Coordinate([
				this.state.x + BigInt(1) + BigInt(1),
				this.state.y + BigInt(1),
				this.state.z + BigInt(1) + BigInt(1),
				BigInt(0) + BigInt(1)
			]);
		...
		render() {
			...
			for(var i = 0; i < this.state.output_lines.length; i++){
				output_items.push(<div>{this.state.output_lines[i]}</div>);
			}
			...
			return (
				<div>
					<div style={{textAlign:"right"}}><button style={{fontSize: "200%", backgroundColor: current_ws_colour}}>{this.state.websocket_url}, State = {current_ws_state}</button></div>
					<h1>Block Manager, x={this.state.x.toString()}, y={this.state.y.toString()}, z={this.state.z.toString()}</h1>
					<BlockGrid
						x={this.state.x}
						y={this.state.y}
						z={this.state.z}
					...
					<div>
						<form accept-charset="utf-8">
							<input type="text" id="input" onChange={(e) => this.onInputValueChange(e)} value={this.state.input_value}/>
						</form>
					</div>
					<div>
						<input type="button" value="SEND" onClick={() => this.onSingleBlockWrite()} />
					</div>
					<div>{output_items}</div>
				</div>
			);
		}
	}
