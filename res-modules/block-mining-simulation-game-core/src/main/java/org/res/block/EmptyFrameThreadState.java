...
public class EmptyFrameThreadState extends UserInterfaceFrameThreadState {
	...
	public void reprintFrame() throws Exception {
		...
		this.printTextAtScreenXY(new ColouredTextFragment(theText, new int [] {DEFAULT_TEXT_BACKGROUND_COLOR}), this.getFrameWidth() > theText.length() ? ((this.getFrameWidth() - theText.length()) / 2L) : 0L, this.getFrameHeight() / 2L, true);
	}
