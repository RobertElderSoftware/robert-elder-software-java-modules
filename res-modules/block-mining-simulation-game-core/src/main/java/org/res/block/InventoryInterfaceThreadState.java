...
public class InventoryInterfaceThreadState extends UserInterfaceFrameThreadState implements RenderableListContainer {
	...
	public InventoryInterfaceThreadState(BlockManagerThreadCollection blockManagerThreadCollection, ClientBlockModelContext clientBlockModelContext, ConsoleWriterThreadState consoleWriterThreadState) throws Exception {
		...
		if(action == null){
			...
			switch(action){
				case ACTION_QUIT:{
					...
					logger.info("Inventory frame, discarding keyboard input: " + actionString);
				}
			}
		}
	}
