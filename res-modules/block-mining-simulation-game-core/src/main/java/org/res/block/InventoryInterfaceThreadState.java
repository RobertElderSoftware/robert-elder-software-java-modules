...
public class InventoryInterfaceThreadState extends UserInterfaceFrameThreadState {
	...
	public void reprintFrame() throws Exception {
		...
		this.printTextAtScreenXY(new ColouredTextFragment("- Inventory -", new int[] {RESET_BG_COLOR}), 10L, 0L, true);
		...
		if(inventory != null){
			...
			if(itemStacks.size() == 0){
				this.printTextAtScreenXY(new ColouredTextFragment("Empty.", new int[] {RESET_BG_COLOR}), inventoryItemsXOffset, 2L, true);
			}else{
				for(int i = 0; i < itemStacks.size(); i++){
					...
					this.printTextAtScreenXY(new ColouredTextFragment(inventoryItemText, new int[] {RESET_BG_COLOR}), printTextX, printTextY, true);
				}
			}
		}
	}
